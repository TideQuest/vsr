...
function sqrt(n) {
    if (n < 0n)
        throw new Error('sqrt: input must be a non-negative bigint');
    if (n === 1n)
        return n;
    let b = 1n << BigInt(2 * n.toString().length);
    for (let a = (n / b + b) >> 1n; b !== a && b !== a - 1n;) {
        b = a;
        a = (n / b + b) >> 1n;
    }
    return b;
 ...
 */
function randomBits(bits, randFn = utils_1.randomBytes) {
    const bytes = Math.ceil(bits / 8);
    const n = BigInt('0x' + (0, utils_1.bytesToHex)(randFn(bytes)));
