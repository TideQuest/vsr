...
exports.millerRabinBaseTest = millerRabinBaseTest;
exports.jacobi = jacobi;
exports.lucas = lucas;
exports.bailliePSW = bailliePSW;
exports.isProbablePrime = isProbablePrime;
exports.isProbablePrimeRSA = isProbablePrimeRSA;
exports.isProbablySafePrime = isProbablySafePrime;
 ...
 */
function millerRabin(w, iterations, randFn = utils_1.randomBytes) {
    if (typeof w !== 'bigint')
        throw new Error('number expected to be bigint');
    if (!Number.isSafeInteger(iterations))
        throw new Error('number of iterations should be safe interger');
    if (typeof randFn !== 'function')
        throw new Error('randFn should be function');
    if (w < 2n)
        return false;
    ...
    if (2n ** BigInt(a) * m !== w - 1n)
        throw new Error('millerRabin: wrong assertion');
    const wlen = w.toString(2).length; // 3. _wlen_ = **len** ( _w_ ).
    step4: for (let i = 1; i <= iterations; i++) {
        // Step 4.1 + 4.2
        let b = 0n;
        while (b <= 1n || b >= w - 1n)
            b = (0, utils_js_1.randomBits)(wlen, randFn);
        let z = (0, utils_js_1.pow)(b, m, w); // Step 4.3
        if (z === 1n || z === w - 1n)
            continue; // Step 4.4
        // Step 4.5
        for (let j = 1; j <= a - 1; j++) {
            z = (z * z) % w; // Step 4.5.1
            if (z === w - 1n)
                continue step4; // Step 4.5.2 + 4.7 (continue 4)
            if (z === 1n)
                return false; // 4.5.3 + 4.6
        }
        return false;
    }
    return true;
...
function millerRabinBaseTest(w, base) {
    return millerRabin(w, 1, (len) => (0, utils_js_1.numberToBytes)(base, len));
}
/**
...
function jacobi(a, n) {
    ...
    let s = 1;
    if (e % 2 !== 0) {
        const mod8 = (0, utils_js_1.mod)(n, 8n);
        if (mod8 === 1n || mod8 === 7n)
            s = 1;
        else if (mod8 === 3n || mod8 === 5n)
            s = -1;
    }
    if ((0, utils_js_1.mod)(n, 4n) === 3n && (0, utils_js_1.mod)(a1, 4n) === 3n)
        s = -s; // Step 6
    const n1 = (0, utils_js_1.mod)(n, a1); // Step 7
...
function lucas(C) {
    ...
    for (;; D = -(D + (D > 0n ? 2n : -2n))) {
        const js = jacobi(D, C);
        if (js === 0)
            return false; // if jacobi symbol = 0 for any D in the sequence, return (COMPOSITE)
        // GCD check added in FIPS186-5
        if (js === -1 && (0, utils_js_1.gcd)(C, (1n - D) / 4n) === 1n)
            break;
    }
    const K = C + 1n; // Step 3
    ...
    // Computes (A * (C + 1) / 2) % C
    const div2 = (A, C) => (0, utils_js_1.mod)(A * ((C + 1n) >> 1n), C);
    // Step 6
    for (let i = BigInt(r - 1); i >= 0n; i--) {
        const Utemp = (0, utils_js_1.mod)(Ui * Vi, C); // Step 6.1
 ...
 */
function isProbablePrime(n, iters, randFn = utils_1.randomBytes) {
    const sieveRes = checkSieve(n);
    if (sieveRes !== undefined)
        return sieveRes;
    if (!millerRabin(n, iters, randFn))
        return false;
    return lucas(n);
}
function isProbablePrimeRSA(n, randFn = utils_1.randomBytes) {
 ...
 */
function isProbablySafePrime(p, iters, randFn = utils_1.randomBytes) {
    if (!isProbablePrime(p, iters, randFn))
        return false;
    const q = (p - 1n) / 2n;
