 ...
 */
function randomBigInt(bytes, min, max, randFn = randomBytes) {
    let res;
    do
        res = bytesToNumber(randFn(bytes));
    while (res < min || res >= max); // Key [2, p-1)
    return res;
}
export function genElGamalParams(bits) {
    ...
    let p = 0n;
    do
        p = bytesToNumber(randomBytes(bits / 8));
