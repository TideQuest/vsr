...
import { gcd, mod, numberToBytes, pow, randomBits } from './utils.js';
 ...
 */
export function millerRabin(w, iterations, randFn = randomBytes) {
    if (typeof w !== 'bigint')
...
export function millerRabinBaseTest(w, base) {
...
export function lucas(C) {
    ...
    // Computes (A * (C + 1) / 2) % C
    const div2 = (A, C) => mod(A * ((C + 1n) >> 1n), C);
    // Step 6
 ...
 */
export function isProbablePrime(n, iters, randFn = randomBytes) {
    const sieveRes = checkSieve(n);
    if (sieveRes !== undefined)
        return sieveRes;
...
export function isProbablePrimeRSA(n, randFn = randomBytes) {
 ...
 */
export function isProbablySafePrime(p, iters, randFn = randomBytes) {
