...
async function packClientHello({ host, sessionId = crypto_1.crypto.randomBytes(32), random = crypto_1.crypto.randomBytes(32), keysToShare, psk, cipherSuites, supportedProtocolVersions = Object
    .keys(constants_1.TLS_PROTOCOL_VERSION_MAP), signatureAlgorithms = Object
    .keys(constants_1.SUPPORTED_SIGNATURE_ALGS_MAP), applicationLayerProtocols = [] }) {
    // generate random & sessionId if not provided
    const packedSessionId = (0, packets_1.packWithLength)(sessionId).slice(1);
    const cipherSuiteList = (cipherSuites || Object.keys(constants_1.SUPPORTED_CIPHER_SUITE_MAP)).map(cipherSuite => constants_1.SUPPORTED_CIPHER_SUITE_MAP[cipherSuite].identifier);
